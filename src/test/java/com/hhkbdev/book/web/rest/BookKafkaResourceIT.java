...
import com.hhkbdev.book.config.EmbeddedKafka;
import com.hhkbdev.book.config.KafkaSseConsumer;
import com.hhkbdev.book.config.KafkaSseProducer;
...
class BookKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/book-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/book-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/book-kafka/unregister"));
                return;
            }
        }
